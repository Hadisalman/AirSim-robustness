...
# by default, it is set to "airsim_robustness:10.0-devel-ubuntu18.04"
# else user can specify a docker image as follows:
# $ ./run_docker_image.sh DOCKER_IMAGE_NAME:TAG
DOCKER_IMAGE_NAME=${1:-airsim_robustness:10.0-devel-ubuntu18.04}
...
nvidia-docker run -it \
    ...
    # /bin/bash 
    # --env="XDG_RUNTIME_DIR=/tmp/run/user/app" \


# docker run --runtime=nvidia -ti --rm airsim_robustness:vulkan-new /bin/bash    