...
class ped_recognition_dataset(object):
    def __init__(self, num_samples, dataset_path='airsim_dataset'):
        ...
        assert found
        found = self.client.simSetSegmentationObjectID(mesh_name=self.ped_object_name, object_id=25)
        assert found
    ...
    def generate(self, print_freq=100):
        while self.curr_idx < self.num_samples and not self.stop_generation:
...
if __name__ == "__main__":
    ...
    def kill_generation(signal, frame):
        dataset.stop_generation = True
        print('[Stopping data generation.]')
        time.sleep(1)
        dataset.client.reset()
        
    signal.signal(signal.SIGINT, kill_generation)
    dataset.generate()
